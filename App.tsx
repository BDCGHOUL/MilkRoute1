    ...
    <div className="flex flex-col h-full w-full bg-black text-white overflow-hidden p-3 gap-3 relative">
      ...
      {isAdmin && (
        <div className="fixed bottom-28 right-8 flex flex-col gap-4 z-[2000]">
          <button onClick={() => setIsClosureMode(!isClosureMode)} className={`w-14 h-14 rounded-2xl border-2 flex items-center justify-center transition-all ${isClosureMode ? 'bg-red-600 border-white' : 'bg-zinc-900 border-zinc-800'}`}><Construction size={22} /></button>
