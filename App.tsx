  ...
  const generateBriefing = async () => {
    ...
    try {
      ...
      const searchResponse = await ai.models.generateContent({
        ...
        contents: `Research current and planned roadworks in Cambridge tonight using official sources. 
        Focus strictly on the following streets: ${streetNames}. 
        Provide a professional driver summary and advice on how to handle the route (e.g., skip a street, skip a call, or approach from a different direction). 
      ...
      // Step 2: Auto-parse closures from text
      const parserResponse = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Extract coordinates of blocked stops from this report: "${briefingText}". 
        STOPS: ${JSON.stringify(currentRouteStops.map(s => ({ addr: s.addr, lat: s.lat, lng: s.lng })))}
        Return a JSON array of {lat, lng, note} for actual blockages found.`,
        config: {
      ...
      setAiBriefing({ text: "Intel system unavailable. Use manual road monitoring.", sources: [] });
    } finally {
    ...
    <div className="flex flex-col h-full w-full bg-black text-white overflow-hidden p-3 gap-3 relative">
      ...
      <div className="flex justify-between items-center h-14 flex-shrink-0 px-3 z-[50] glass rounded-2xl">
        ...
        <div className="flex gap-4">
            <button onClick={() => setActiveModal('BRIEFING')} className="p-2 text-zinc-500 hover:text-blue-400"><Info size={18} /></button>
      ...
      <div className="flex-1 relative rounded-3xl overflow-hidden border border-zinc-800 bg-zinc-900 shadow-2xl">
        ...
        <div className="absolute top-6 right-6 flex flex-col gap-3 z-[1000] items-end">
           <button onClick={() => { const s = stops[index]; if (s) window.open(`https://www.google.com/maps/dir/?api=1&destination=${s.lat},${s.lng}`, '_blank'); }} className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-sm pulse-accent">LAUNCH NAVIGATION</button>
           <button onClick={optimizeRemainingRoute} disabled={isOptimizing} className="px-8 py-4 bg-zinc-900 border border-zinc-700 rounded-2xl font-black text-xs flex items-center gap-2">{isOptimizing ? <Loader2 size={16} className="animate-spin" /> : <Zap size={16} className="text-yellow-400" />} RE-ROUTE</button>
        </div>
      </div>

      {isAdmin && (
        <div className="fixed bottom-28 right-8 flex flex-col gap-4 z-[2000]">
          <button onClick={() => setIsClosureMode(!isClosureMode)} className={`w-14 h-14 rounded-2xl border-2 flex items-center justify-center ${isClosureMode ? 'bg-red-600 border-white' : 'bg-zinc-900 border-zinc-800'}`}><Construction size={22} /></button>
          <button onClick={() => setActiveModal('ADD_STOP')} className="w-14 h-14 bg-white text-black rounded-2xl flex items-center justify-center"><Plus size={32} /></button>
        </div>
      )}
      ...
      {activeModal === 'BRIEFING' && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 bg-black/90 backdrop-blur-xl">
          <div className="glass border-blue-500/20 rounded-[2.5rem] p-10 w-full max-w-lg shadow-2xl flex flex-col max-h-[85vh]">
             <div className="flex items-center gap-3 mb-8 shrink-0">
                <Sparkles size={28} className="text-blue-500" />
                <h2 className="text-3xl font-black uppercase italic">Road Intel</h2>
             </div>
             {isBriefingLoading ? (
               <div className="flex flex-col items-center py-20 gap-6 grow justify-center"><Loader2 size={48} className="text-blue-500 animate-spin" /><p className="text-zinc-500 font-black uppercase tracking-widest text-xs">Syncing Feeds...</p></div>
             ) : (
               <>
                 <div className="flex-1 overflow-y-auto pr-4 no-scrollbar space-y-6">
                   <div className="text-lg font-medium text-zinc-300 leading-relaxed border-l-2 border-blue-500/50 pl-6 italic">{aiBriefing?.text}</div>
                   <div className="flex flex-wrap gap-2">{aiBriefing?.sources.map((s, i) => <a key={i} href={s.uri} target="_blank" className="text-[9px] bg-blue-500/10 text-blue-400 px-2 py-1 rounded border border-blue-500/20">{s.title}</a>)}</div>
                 </div>
                 <button onClick={() => setActiveModal('NONE')} className="mt-6 w-full bg-white text-black py-4 rounded-2xl font-black uppercase tracking-widest">Acknowledge</button>
               </>
             )}
          </div>
        </div>
      ...
      {activeModal === 'ADMIN_LOGIN' && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="glass rounded-[2rem] p-10 w-full max-w-xs text-center">
            <h2 className="text-xl font-black uppercase mb-8">Access</h2>
            <form onSubmit={handleAdminSubmit}>
              <input type="password" value={adminPassInput} onChange={(e) => setAdminPassInput(e.target.value)} className="w-full bg-black border-2 border-zinc-800 rounded-2xl py-6 text-center text-4xl font-black mb-8 mono" autoFocus />
              <button type="submit" className="w-full bg-white text-black py-4 rounded-2xl font-black">LINK</button>
            </form>
          </div>
        </div>
      )}
